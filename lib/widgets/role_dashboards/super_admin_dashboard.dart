import 'package:flutter/material.dart';
import 'package:get/get.dart';
import '../../controllers/dashboard_controller.dart';
import '../stat_card.dart';
import '../dashboard_section.dart';
import '../action_tile.dart';

class SuperAdminDashboard extends GetView<DashboardController> {
  const SuperAdminDashboard({super.key});

  @override
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        DashboardSection(
          title: 'System Overview',
          child: Column(
            children: [
              Row(
                children: [
                  Expanded(child: StatCard(title: 'Total Revenue', value: '\$450,000')),
                  Expanded(child: StatCard(title: 'Active Clients', value: '1,240')),
                ],
              ),
              const SizedBox(height: 16),
              Row(
                children: [
                  Expanded(child: StatCard(title: 'System Uptime', value: '99.9%')),
                  Expanded(child: StatCard(title: 'New Signups', value: '+45')),
                ],
              ),
            ],
          ),
        ),
        const SizedBox(height: 24),
        DashboardSection(
          title: 'Recent System Logs',
          child: Card(
            child: ListView.separated(
              shrinkWrap: true,
              physics: const NeverScrollableScrollPhysics(),
              itemCount: 3,
              separatorBuilder: (context, index) => const Divider(height: 1),
              itemBuilder: (context, index) {
                final logs = [
                  'Database backup completed successfully',
                  'New server instance deployed in us-east-1',
                  'Critical security patch applied'
                ];
                return ActionTile(
                  icon: Icons.dns,
                  iconColor: Colors.blue,
                  title: logs[index],
                  subtitle: 'Today, ${10 + index}:00 AM',
                );
              },
            ),
          ),
        ),
        const SizedBox(height: 24),
        DashboardSection(
          title: 'System Access',
          child: Column(
            children: [
              ActionTile(
                icon: Icons.business_rounded,
                title: 'Companies',
                subtitle: 'Manage all registerd organizations',
                onTap: controller.openCompanies,
              ),
              ActionTile(
                icon: Icons.location_city_rounded,
                title: 'Branches',
                subtitle: 'Manage company branch locations',
                onTap: controller.openBranches,
              ),
              ActionTile(
                icon: Icons.account_tree_rounded,
                title: 'Departments',
                subtitle: 'Manage organization hierarchy',
                onTap: controller.openDepartments,
              ),
              ActionTile(
                icon: Icons.inventory_2_rounded,
                title: 'Assets',
                subtitle: 'Track and allocate company inventory',
                onTap: controller.openAssets,
              ),
              ActionTile(
                icon: Icons.payments_rounded,
                title: 'Pay Grade & Salary',
                subtitle: 'Configure salary structures',
                onTap: controller.openSalary,
              ),
              ActionTile(
                icon: Icons.calendar_month_rounded,
                title: 'Financial Year',
                subtitle: 'Manage fiscal periods',
                onTap: controller.openSettings, // Or a specific view
              ),
              ActionTile(
                icon: Icons.history_rounded,
                title: 'Audit Logs',
                subtitle: 'View system-wide activity',
                onTap: controller.openAuditLogs,
              ),
              ActionTile(
                icon: Icons.groups_rounded,
                title: 'Employee Management',
                subtitle: 'Manage all company staff',
                onTap: controller.openDirectory,
              ),
              ActionTile(
                icon: Icons.admin_panel_settings_rounded,
                title: 'User Management',
                subtitle: 'Configure roles and permissions',
                onTap: controller.openSettings,
              ),
            ],
          ),
        ),
      ],
    );
  }
}
